    ...
    <div class="col-md-4 col-md-offset-2">
              ...
              <div class="box-body">
                <div class="form-group">
                  <div class="col-sm-9">
       <select name="status" class="form-control" required id="class">
          <option value=""> --- Select Student Status --- </option>
		  <?php 
		  $inm6 = mysqli_query($conn,"select * from status");
				$i=1;
			while($inm7= mysqli_fetch_array($inm6,MYSQLI_ASSOC)){ ?>
						<option value="<?php echo $inm7['code'];?>"><?php echo $inm7['name'];?></option>
				<?php	}
			?>
        </select>                  </div>
                </div>
	 ...
	 if(isset($_POST["status"])){
		 ...
		 ?>
            ...
            <div class="box-header">
              <h3 class="box-title">Status:&nbsp;&nbsp;<?php if($status == 1){ ?> Active <?php } else{ ?> Inactive <?php } ?></h3>&nbsp;&nbsp;
            </div>
              <table id="example1" class="table table-bordered table-striped">
                ...
                <tbody>
				<?php 
		     $student = mysqli_query($conn,'SELECT * FROM student where status ="'.$status.'"');
			 $i=1;
